...
const ContactForm = () => {
  ...
  const services = [
    { id: 'Desarrollo de Ecosistemas Web de Élite', title: t('contact.service_web_title'), desc: t('contact.service_web_desc'), icon: <Globe className="w-5 h-5" /> },
    { id: 'Ingeniería de Automatizaciones con IA', title: t('contact.service_ai_title'), desc: t('contact.service_ai_desc'), icon: <Zap className="w-5 h-5" /> },
    { id: 'Estrategia de Growth & Ads', title: t('contact.service_growth_title'), desc: t('contact.service_growth_desc'), icon: <Rocket className="w-5 h-5" /> },
    { id: 'Solución Integral (Full Stack Business)', title: t('contact.service_full_title'), desc: t('contact.service_full_desc'), icon: <Layers className="w-5 h-5" /> }
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus('success');
        toast.success(t('contact.success', { name: formData.name }), {
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] h-[650px] flex flex-col overflow-hidden border-blue-500/20 shadow-2xl">
              <div className="p-6 border-b border-white/5 flex justify-between bg-black/40">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-blue-500 text-[10px] uppercase font-bold tracking-widest">{t('contact.ai_label')}</span>
                </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white/[0.01] border border-white/5 p-8 md:p-12 rounded-[3rem] shadow-2xl h-full flex flex-col justify-between">
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  <p className="text-[10px] text-gray-600 uppercase tracking-widest font-bold ml-1">{t('contact.interest_label')}</p>
              ...
              <div>
                ...
                >
                  {status === 'enviando' ? t('contact.sending') : t('contact.submit')}
                  <Send className="w-4 h-4" />
                </button>

                <p className="mt-4 text-[9px] text-gray-600 text-center leading-relaxed">
                  {t('contact.privacy_text')} <Link to="/aviso-legal" className="text-blue-500/80 underline hover:text-blue-400 transition-colors">{t('contact.privacy_link')}</Link>.
                </p>
              </div>
